...
OUT=ROOT/'03_text/matched/switch_pc_v5'
...
def main():
 ...
 summ={'switch_messages':len(sw),'safe_total':len(matches),'safe_coverage':round(100*len(matches)/len(sw),3),'remaining':len(un),'review_forced_used':sum(1 for r in matches if r['pc_status']=='review-forced'),'match_kinds':dict(kinds),'statuses':dict(statuses)}
