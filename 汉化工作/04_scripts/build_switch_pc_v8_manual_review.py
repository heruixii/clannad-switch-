...
SUP3=ROOT/'03_text/matched/pc_control_parallel_v4/review_signature_manual_accept.tsv'
OUT=ROOT/'03_text/matched/switch_pc_v8_base'
...
def main():
 sw=rd(SW); pc0=rd(PC); sup=rd(SUP); sup2=rd(SUP2); sup3=rd(SUP3)
 ...
 for r in sup3:
  ...
  e=bykey.get(k)
  if e and not e['zh_text']:
   e['zh_text']=r['zh_text'];e['status']='review-forced-manual'
 ...
 summ={'switch_messages':len(sw),'safe_total':len(matches),'safe_coverage':round(100*len(matches)/len(sw),3),'remaining':len(un),'review_forced_used':sum(1 for r in matches if r['pc_status']=='review-forced'),'review_equal_used':sum(1 for r in matches if r['pc_status']=='review-equal-strict'),'review_manual_used':sum(1 for r in matches if r['pc_status']=='review-forced-manual'),'match_kinds':dict(kinds),'statuses':dict(statuses)}
