...
R=Path(r'D:\switch游戏\个人汉化\clannad\汉化工作'); ORIG=R/'03_text/ui_work/OTHCG.PAK'; WORK=R/'05_build/othcg_chs_unpacked'; OUT=R/'05_build/OTHCG.PAK.out'
...
sha=hashlib.sha256(rb).hexdigest().upper(); print('REPACK',fc,'SIZE',len(rb),'BAD',len(bad),'SHA256',sha)
(R/'05_build/othcg_repack_verify.json').write_text(json.dumps({'entries':fc,'size':len(rb),'bad':bad,'sha256':sha},indent=2),encoding='utf-8')
