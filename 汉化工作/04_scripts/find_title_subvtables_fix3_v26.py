...
for ti,label in [(0x23d380,'cScrSelect'),(0x23d3a0,'cLabelSelect')]:
 ...
 for h in hits:
  print('REF',hex(h))
  for o in range(max(0,h-0x20),min(len(img),h+0x160),8):
   v=struct.unpack_from('<Q',img,o)[0];print(hex(o),hex(v)+(' CODE' if 0x30<=v<len(t) else ''))
