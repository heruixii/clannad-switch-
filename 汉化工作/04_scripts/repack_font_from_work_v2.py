...
R=Path(r'D:\switch游戏\个人汉化\clannad\汉化工作'); ORIG=R/'03_text/switch_work/paks/FONT.PAK'; WORK=R/'05_build/font_package/FONT.PAK_unpacked'; OUT=R/'05_build/font_package/FONT.PAK.out'
...
rep={'entries':fc,'block':bs,'size':len(rb),'bad':bad,'sha256':hashlib.sha256(rb).hexdigest().upper()}; (R/'05_build/font_package/font_repack_verify.json').write_text(json.dumps(rep,ensure_ascii=False,indent=2),encoding='utf-8'); print(rep); raise SystemExit(1 if bad else 0)
