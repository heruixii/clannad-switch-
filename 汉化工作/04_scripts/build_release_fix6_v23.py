...
R=Path(r'D:\switch游戏\个人汉化\clannad\汉化工作');rel=R/'05_build/CLANNAD_CHS_LayeredFS_v1.0.7_fix6';zipf=R/'05_build/CLANNAD_CHS_LayeredFS_v1.0.7_fix6.zip'
...
full='CF38595316BAA425E792CE5CD122DFC600000000000000000000000000000000';pd=rel/'atmosphere/exefs_patches/CLANNAD_CHS_v1.0.7_fix6';pd.mkdir(parents=True)
sources={
'SCRIPT.PAK':R/'05_build/script_package_fix6/SCRIPT.PAK.out',
...
shutil.copy2(R/'05_build/fix6_exefs/CF38595316BAA425E792CE5CD122DFC6.ips',pd/(full+'.ips'))
def sha(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
  for b in iter(lambda:f.read(8<<20),b''):h.update(b)
 return h.hexdigest().upper()
readme=f'''CLANNAD Nintendo Switch 简体中文补丁 fix6
...
- fix6 恢复第 1 字段为原版英文内部排序键；彩色可见标题、正文A、正文B继续保持完整简中。
...
2. 删除旧 CLANNAD exefs_patches，尤其不要让 fix5 和 fix6 IPS 同时加载。
...
qa=json.loads((R/'05_build/final_release_sources_verify_fix6.json').read_text(encoding='utf-8'));iq=json.loads((R/'05_build/fix6_exefs/FINAL_IPS_QA_fix6.json').read_text(encoding='utf-8'));kq=json.loads((R/'05_build/keyword_fix6/keyword_fix6_qa.json').read_text(encoding='utf-8'))
summary={'release':'fix6','target_version':'v1.0.7','module_id':full,'source_total_bad':qa['TOTAL_BAD'],'ips':iq,'keyword':kq}
(rel/'FINAL_QA_SUMMARY_fix6.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
entries=[]
