...
R=Path(r'D:\switch游戏\个人汉化\clannad\汉化工作');rel=R/'06_release/CLANNAD_CHS_LayeredFS_v1.0.7_fix8';zipf=R/'06_release/CLANNAD_CHS_LayeredFS_v1.0.7_fix8.zip'
...
full='CF38595316BAA425E792CE5CD122DFC600000000000000000000000000000000';pd=rel/'atmosphere/exefs_patches/CLANNAD_CHS_v1.0.7_fix8';pd.mkdir(parents=True)
sources={
...
shutil.copy2(R/'05_build/fix8_exefs/CF38595316BAA425E792CE5CD122DFC6.ips',pd/(full+'.ips'))
def sha(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
  for b in iter(lambda:f.read(8<<20),b''):h.update(b)
 return h.hexdigest().upper()
readme=f'''CLANNAD Nintendo Switch 简体中文补丁 fix8
...
- fix8 将正式流程英文槽原位替换为“朋也”，Tomoya 剩余 0。
...
- fix8 恢复第 1 字段为原版英文内部排序键；彩色可见标题、正文A、正文B继续保持完整简中。
...
安装
====
1. 删除或移走旧 fix1～fix7 的 CLANNAD romfs 替换目录。
2. 删除旧 CLANNAD exefs_patches，尤其不要让任何旧版 CLANNAD IPS 与 fix8 同时加载。
...
- 主角姓名是否显示“朋也”。
- 进入 After Story（后日谈）后剧情正文是否继续保持中文。
'''
(rel/'README.txt').write_text(readme,encoding='utf-8-sig')
shutil.copy2(R/'docs/03-贡献与署名.md',rel/'CREDITS.md')
shutil.copy2(R/'docs/04-最终用户使用说明.md',rel/'使用说明.md')
qa=json.loads((R/'05_build/final_release_sources_verify_fix8.json').read_text(encoding='utf-8'));iq=json.loads((R/'05_build/fix8_exefs/FINAL_IPS_QA_fix8.json').read_text(encoding='utf-8'));kq=json.loads((R/'05_build/keyword_fix6/keyword_fix6_qa.json').read_text(encoding='utf-8'))
summary={'release':'fix8','target_version':'v1.0.7','module_id':full,'source_total_bad':qa['TOTAL_BAD'],'name_qa':iq,'keyword':kq}
(rel/'FINAL_QA_SUMMARY_fix8.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
entries=[]
