...
def cstr(a):
 ...
 o=a-RB;e=rb.find(b'\0',o,min(len(rb),o+2000))
 if e<0:return None
 try:return rb[o:e].decode('utf-8')
 except:return None
refs={}
for i,x in enumerate(ins):
 if not(0x179000<=x.address<0x17C000):continue
 ...
 for j in range(i+1,min(i+5,len(ins))):
  ...
  if y.mnemonic=='add' and len(y.operands)>=3 and y.operands[0].type==ARM64_OP_REG and y.operands[1].type==ARM64_OP_REG and y.operands[0].reg==r and y.operands[1].reg==r and y.operands[2].type==ARM64_OP_IMM:
   ...
   if s and len(re.findall(r'[A-Za-z]',s))>=2:refs.setdefault((a,s),[]).append((x.address,y.address))
   break
print('UNIQUE',len(refs))
for n,((a,s),hs) in enumerate(sorted(refs.items(),key=lambda kv:min(x[0] for x in kv[1]))):print(f'{n:03d}\t{a:08X}\t'+','.join(f'{x:08X}/{y:08X}' for x,y in hs)+f'\t{s!r}')
