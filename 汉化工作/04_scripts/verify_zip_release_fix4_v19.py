...
R=Path(r'D:\switch游戏\个人汉化\clannad\汉化工作');src=R/'05_build/CLANNAD_CHS_LayeredFS_v1.0.7_fix4';z=R/'05_build/CLANNAD_CHS_LayeredFS_v1.0.7_fix4.zip'
...
with zipfile.ZipFile(z,'r') as q:
 ...
 ips=[n for n in names if n.endswith('.ips')];expected_ips=f'atmosphere/exefs_patches/CLANNAD_CHS_v1.0.7_fix4/{full}.ips'
 ...
 old=[n for n in names if any(x in n.lower() for x in ['fix1','fix2','fix3'])]
 ...
 (R/'05_build/FINAL_RELEASE_QA_fix4.json').write_text(json.dumps(rep,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps(rep,ensure_ascii=False,indent=2));raise SystemExit(1 if rep['total_bad'] else 0)
