...
def feat(j,z):
    ha,hb=hans(body(j['text'])),hans(body(z['text']));sh=len(ha&hb);mn=min(len(ha),len(hb));mx=max(len(ha),len(hb));oc=sh/max(mn,1);jc=sh/max(len(ha|hb),1);vm=int(bool(vars_(body(j['text']))&vars_(body(z['text']))));fc=fpc[(j['fp'],z['fp'])];jt=fj[j['fp']];fr=fc/max(jt,1);a,b=sp(j['text']),sp(z['text']);sc=spc[(a,b)] if a and b else 0;sr=sc/max(spp[a],1) if a else 0
    return sh,len(ha),len(hb),oc,jc,vm,fc,jt,fr,sc,sr
...
for sc,pairs in by.items():
    ...
    for p in sorted(pairs):
        ...
        cur.append(p)
    if len(cur)>=12:runs.append((sc,cur))
rng=random.Random(20260913);obs=[]
for c in range(6000):
    sc,run=rng.choice(runs);L=rng.randint(12,min(48,len(run)));st=rng.randint(0,len(run)-L);seg=run[st:st+L];J,Z=get(sc);Js=[J[j] for j,z in seg];Zs=[Z[z] for j,z in seg];truth=[(i,i) for i in range(L)]
    ...
    for a,b in P:
        if why[(a,b)]!='strict-anchor':continue
        obs.append((int((a,b) in T),)+feat(Js[a],Zs[b]))
print('anchors',len(obs),'wrong',sum(1-o[0] for o in obs))
def test(name,p):
    x=[o for o in obs if p(o)];w=sum(1-o[0] for o in x);print(name,'n',len(x),'wrong',w,'precision',round((len(x)-w)/max(len(x),1),6))
# o correct, sh,hj,hz,oc,jc,var,fc,jt,fr,sc,sr
for sh in [2,3,4,5,6,7]:
  for oc in [.5,.6,.7,.8,1.0]:test(f'sh{sh}+ oc{oc}',lambda o,sh=sh,oc=oc:o[1]>=sh and o[4]>=oc)
for jc in [.25,.3,.4,.5,.6]:test('jaccard>='+str(jc),lambda o,jc=jc:o[1]>=3 and o[5]>=jc)
test('var_or_rare',lambda o:o[6] or (o[8]<=500 and o[9]>=.5 and o[7]>=3))
test('var_rare_sh6',lambda o:o[6] or (o[8]<=500 and o[9]>=.5 and o[7]>=3) or o[1]>=6)
test('var_rare_sh4oc80',lambda o:o[6] or (o[8]<=500 and o[9]>=.5 and o[7]>=3) or (o[1]>=4 and o[4]>=.8))
test('var_rare_sh3oc100',lambda o:o[6] or (o[8]<=500 and o[9]>=.5 and o[7]>=3) or (o[1]>=3 and o[4]>=1.0))
